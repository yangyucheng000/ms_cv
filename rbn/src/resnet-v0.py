...
class RepresentativeBatchNorm2d(nn.BatchNorm2d):
    ...
    def __init__(
        ...
        self.gamma_fix = Parameter(
            initializer(gamma_init, num_features), name="gamma", requires_grad=False
        )
        self.beta_fix = Parameter(
            initializer(beta_init, num_features), name="beta", requires_grad=False
        )
        ...
        self.center_weight = Parameter(
            initializer("zeros", num_features), name="beta", requires_grad=affine
        )
        self.scale_weight = Parameter(
            initializer("zeros", num_features), name="beta", requires_grad=affine
        )
        self.scale_bias = Parameter(
            initializer("ones", num_features), name="beta", requires_grad=affine
        )
        self.stas = P.ReduceMean(keep_dims=True)
    ...
    def construct(self, x):
        ...
        x += self.center_weight.view(1, self.num_features, 1, 1) * self.stas(x, (2, 3))
        ...
        if flag:
            ...
            output = self.bn_train(
                x, self.gamma_fix, self.beta_fix, self.moving_mean, self.moving_variance
            )[0]

        else:
            output = self.bn_infer(
                x, self.gamma_fix, self.beta_fix, self.moving_mean, self.moving_variance
            )[0]
        ####### scaling calibration begin #######
        scale_factor = self.sigmoid(
            self.scale_weight.view(1, self.num_features, 1, 1) * self.stas(output, (2, 3)) + self.scale_bias.view(1, self.num_features, 1, 1)
        )
        ####### scaling calibration end #######
        return self.gamma.view(1, self.num_features, 1, 1) * scale_factor * output + self.beta.view(1, self.num_features, 1, 1)
...
def _bn(channel, res_base=False):
    if res_base:
        return RepresentativeBatchNorm2d(
            channel,
            eps=1e-5,
            momentum=0.1,
            gamma_init=1,
            ...
            if self.use_se:
                if stride == 1:
                    self.down_sample_layer = nn.SequentialCell(
                        [
                            _conv1x1(
                                in_channel, out_channel, stride, use_se=self.use_se
                            ),
                            _bn(out_channel),
                        ]
                    )
                ...
                self.down_sample_layer = nn.SequentialCell(
                    [
                        _conv1x1(in_channel, out_channel, stride, use_se=self.use_se),
                        _bn(out_channel),
                    ]
                )
...
class ResidualBlockBase(nn.Cell):
    ...
    def __init__(
        ...
        self.conv1 = _conv3x3(
            in_channel, out_channel, stride=stride, res_base=self.res_base
        )
        self.bn1d = _bn(out_channel)
        self.conv2 = _conv3x3(
            out_channel, out_channel, stride=1, res_base=self.res_base
        )
